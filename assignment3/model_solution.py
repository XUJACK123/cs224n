...
class CausalAttention(nn.Module):
    ...
    def forward(
        ...
        Q = Q.view(batch, T, self.n_heads, self.d_attention).transpose(1,2)
        K = K.view(batch, T, self.n_heads, self.d_attention).transpose(1,2)
        V = V.view(batch, T, self.n_heads, self.d_attention).transpose(1,2)
        ...
        out = out.contiguous().view(batch, T, d_model)
        out = self.W_o(out)
        return out
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        batch, seq_size = x.shape
        pos = torch.arange(0, seq_size, device = x.device)
